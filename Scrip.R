...
En este proyecto vamos a trabajar los contenidos que entraban en el primer parcial de pr√°cticas de la asignatura trabajaremos 
...
Podemos seleccionar m·s de una caracterÌstica en concreto:
...
Seleccion 5 ˙ltimas filas:

```{r}
Galapagos %>% slice_tail(n=5)
```

Otra aplicaciÛn interesante de slice es slice_sample() que sirve para seleccionar un n˙mero determiando de filas al azar:

```{r}
Galapagos %>% slice_sample(n=5)
```  
Tambien podemos ordenar en funciÛn del que destaque mas o menos de en una caracteristica, por ejemplo, ordenamos en funcion
...
En este caso vamos a aÒadir una columna que se llame temperatura_F (T∫ en Fahrenheit) a partir de la columna Av_Temp.

Para pasar de ∫C a ∫F: ∫C*9/5+32
...
9. Si queremos la media de los valores de una columna numÈrica utilizamos summarise()
